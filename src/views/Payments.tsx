...
export default function Payments() {
  ...
  // Mutation to persist UPI details permanently in Firestore
  const saveUpiMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('UPI details saved to database settings');
    },
    onError: (err: any) => {
      toast.error('Failed to save UPI details: ' + err.message);
    }
  });

  const openQrModal = (payment: Payment | null) => {
    setQrPayment(payment);
    setQrUpiId(settings?.upiId || '');
    setQrPayeeName(settings?.hostelName || 'Boys PG Hostel');
    if (payment) {
      setQrAmount(payment.balance);
      setQrNote(`Rent ${format(new Date(payment.dueDate), 'MMMM')} - ${payment.tenantName}`);
    } else {
      setQrAmount('');
      setQrNote('Hostel Rent');
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex space-x-2">
          <button 
            className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center hover:bg-indigo-700 transition-colors shadow-sm active:scale-95 transition-all"
            onClick={() => openQrModal(null)}
          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div>
                <h3 className="text-xl font-bold text-slate-900 tracking-tight flex items-center">
                  <QrCode className="h-5 w-5 mr-2 text-indigo-600 animate-pulse" />
                  UPI Rent Payment QR Code
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  {qrPayment ? `Generate custom pay-code for ${qrPayment.tenantName}` : 'Generate general rent payment code for all tenants'}
                </p>
              </div>
              <button 
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest border-b border-slate-100 pb-2">
                  UPI Configuration
                </h4>
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="e.g. name@bank, phone@upi"
                    className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-mono text-slate-800"
                  />
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Hostel Name / Owner Name"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      type="number" 
                      value={qrAmount}
                      onChange={(e) => setQrAmount(e.target.value === '' ? '' : Number(e.target.value))}
                      placeholder="e.g. 5000"
                      className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-bold text-slate-800"
                    />
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-col space-y-2">
                  <p className="text-[10px] text-slate-400 italic">
                    Customize fields above to modify the QR Code instantly. Click below to persist this UPI ID & Payee Name in Firestore so it's loaded as default everywhere!
                  </p>
                  <button
                    type="button"
                    onClick={() => saveUpiMutation.mutate({ upiId: qrUpiId, payeeName: qrPayeeName })}
                    disabled={!qrUpiId || saveUpiMutation.isPending}
                    className="w-full bg-slate-100 text-slate-700 hover:bg-slate-200 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center space-x-2 disabled:opacity-50"
                  >
                    ...
                    <span>Save to Settings (Firestore)</span>
                  </button>
                </div>
              </div>

              {/* QR Code Presentation */}
              <div className="flex flex-col items-center justify-center bg-slate-50 p-6 border border-slate-100 rounded-2xl relative">
                {qrUpiId.trim() ? (
                  <div className="w-full flex flex-col items-center text-center space-y-4">
                    <div className="bg-white p-4 rounded-2xl shadow-md border border-slate-150 inline-block relative overflow-hidden group">
                      <img 
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(
                          `upi://pay?pa=${qrUpiId.trim()}&pn=${encodeURIComponent(qrPayeeName.trim())}&cu=INR${qrAmount ? `&am=${qrAmount}` : ''}${qrNote ? `&tn=${encodeURIComponent(qrNote.trim())}` : ''}`
                        )}`}
                    ...
                    <div>
                      <span className="bg-indigo-150/15 text-indigo-700 text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100">
                        Scan to Pay with BHIM/GPay/PhonePe
                      </span>
                    </div>

                    <div className="space-y-1 w-full text-slate-700">
                      <p className="text-xs font-bold text-slate-800">
                        Payee: <span className="font-semibold text-slate-600">{qrPayeeName || 'Hostel Account'}</span>
                      </p>
                      <p className="text-xs font-bold text-slate-800 font-mono">
                        VPA: <span className="text-slate-600 select-all">{qrUpiId}</span>
                      </p>
                      {qrAmount && (
                        <p className="text-base font-black text-indigo-600 tracking-tight mt-2">
                          Amount: ₹{qrAmount.toLocaleString()}
                        </p>
                      )}
                      {qrNote && (
                    ...
                    <div className="pt-4 border-t border-slate-200/60 w-full flex justify-center space-x-2">
                      <a 
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center shadow-sm"
                      >
                  ...
                  <div className="text-center p-6 space-y-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-800 text-sm">UPI ID Required</p>
                      <p className="text-xs text-slate-400 max-w-xs mt-1">
                        Please enter your Payee UPI ID (VPA) in the settings panel to generate a printable Scan-to-Pay QR code.
                      </p>
                    </div>
                  </div>
                )}
